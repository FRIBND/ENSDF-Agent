...
Run order, see main():
  ...
  3. sync_agent_file() - rebuild agents/ENSDF-Agent.agent.md from two source
                         ...
                         don't support agent-scoped hooks - see hooks.json
    ...
    4. sync_plugin_hooks() - build the plugin-root hooks.json from the source
                                                 workspace hook JSON and agent frontmatter hooks,
                                                 deduplicate entries, and replace source-relative
                                                 .github/hooks paths with ${PLUGIN_ROOT}/hooks paths.
    5. sync()            - mirror-copy the rest of managed_entries (hooks/,
...
The generic sync() loop does not manage these files:
  - preserved_plugin_files: plugin-only files with no source equivalent
        (e.g. plugin.json, icon.png) - edited by hand
    directly in the plugin.
    - hooks.json: generated by sync_plugin_hooks(), not copied from source.
...
def parse_frontmatter_scalar(value: str):
    """Parse the simple scalar values used by hook entries in agent frontmatter."""
    value = value.strip()
...
def rewrite_plugin_hook_path(command: str) -> str:
    """Make a source hook command valid after plugin installation.

    Source commands point into ``.github/hooks`` inside the workspace. Plugin
    hooks run from an external install directory, so bundled files must use
    ``${PLUGIN_ROOT}`` and be quoted for paths containing spaces.
    """
    command = command.replace("\\", "/")
    command = command.replace(".github/hooks/", "${PLUGIN_ROOT}/hooks/")
    return re.sub(
        r'(?<!["\'])\$\{PLUGIN_ROOT\}/hooks/[^\s"\']+',
        r'"\g<0>"',
        command,
    )


def rewrite_hook_entry(entry: dict) -> dict:
    rewritten = {}
    for key, value in entry.items():
        if key in {"command", "windows", "linux", "osx"} and isinstance(value, str):
            rewritten[key] = rewrite_plugin_hook_path(value)
        else:
            rewritten[key] = value
    return rewritten


def append_hook_config(target: dict, source: dict) -> None:
    """Append hook entries while preserving order and removing duplicates."""
    ...
    for event, entries in source_hooks.items():
        ...
        destination = target.setdefault(event, [])
        existing = {
        ...
        for entry in entries:
            if not isinstance(entry, dict):
                raise ValueError(f"Hook event {event!r} contains a non-object entry")
            rewritten = rewrite_hook_entry(entry)
            marker = json.dumps(rewritten, sort_keys=True, separators=(",", ":"))
...
def extract_agent_frontmatter(agent_text: str) -> list[str]:
...
def parse_agent_hook_config(agent_text: str) -> dict:
    """Read flat command hooks from the agent YAML frontmatter.

    The project hook entries use a small, stable YAML subset. Keeping this
    parser local avoids adding a PyYAML dependency to a maintenance script.
    """
    frontmatter = extract_agent_frontmatter(agent_text)
    ...
    def finish_entry() -> None:
        ...
        if current_event is not None and current_entry is not None:
            events.setdefault(current_event, []).append(current_entry)
        current_entry = None
    ...
    for line in frontmatter:
        ...
        event_match = re.match(r"^  ([A-Za-z][A-Za-z0-9_-]*):\s*$", line)
        if event_match:
            finish_entry()
            current_event = event_match.group(1)
            continue

        entry_match = re.match(r"^    -\s+type:\s*(.+?)\s*$", line)
        if entry_match:
            finish_entry()
            current_entry = {"type": parse_frontmatter_scalar(entry_match.group(1))}
            continue

        field_match = re.match(r"^      ([A-Za-z][A-Za-z0-9_-]*):\s*(.*?)\s*$", line)
        if field_match and current_entry is not None:
            current_entry[field_match.group(1)] = parse_frontmatter_scalar(field_match.group(2))

    finish_entry()
    return {"hooks": events}


def build_plugin_hook_config(source_root: Path) -> dict:
    """Build plugin hooks from workspace config plus agent-scoped hooks."""
    merged: dict[str, list[dict]] = {}
    workspace_config = source_root / "hooks" / "block-root-file-creation.json"
    if workspace_config.is_file():
        with workspace_config.open(encoding="utf-8") as handle:
            append_hook_config(merged, json.load(handle))

    agent_file = source_root / "agents" / "ENSDF-Agent.agent.md"
    if agent_file.is_file():
        append_hook_config(merged, parse_agent_hook_config(agent_file.read_text(encoding="utf-8")))

    if not merged:
        raise FileNotFoundError(f"No source hook definitions found under {source_root}")
    return {"hooks": merged}


def sync_plugin_hooks(source_root: Path, plugin_root: Path, dry_run: bool) -> None:
    """Regenerate plugin-root hooks.json from current source hook definitions."""
    target = plugin_root / "hooks.json"
    config = build_plugin_hook_config(source_root)
    generated = json.dumps(config, indent=2) + "\n"
    existing = target.read_text(encoding="utf-8") if target.is_file() else None

    if existing == generated:
        print("SKIP   hooks.json (unchanged)")
    elif dry_run:
        print("COPY   hooks.json (generated from source hooks)")
    else:
        target.write_text(generated, encoding="utf-8")
        print("COPIED hooks.json (generated from source hooks)")
...
def strip_agent_hooks_block(agent_text: str) -> str:
    ...
    for any active agent via hooks.json instead. The workspace-only
...
def build_merged_agent_content(agent_text: str, instructions_text: str) -> str:
    ...
    Two things happen to agent_text before it's used:
      1. Its agent-scoped ``hooks:`` frontmatter block is stripped via
         ...
         plugin; equivalent hooks live in hooks.json instead.
...
def main() -> int:
    ...
    sync_plugin_hooks(source_root, plugin_root, args.dry_run)
